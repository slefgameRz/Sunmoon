  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-cyan-100 p-4 md:p-8 lg:p-12">
      <header className="mb-8 text-center">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-gray-800 font-serif">
          พยากรณ์น้ำและสภาพอากาศ
        </h1>
        <p className="mt-2 text-lg md:text-xl text-gray-600">ข้อมูลสำคัญเกี่ยวกับน้ำขึ้นน้ำลง สภาพอากาศ และระดับน้ำ</p>
      </header>
      ...
      <main className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <Card className="col-span-full bg-white shadow-xl border-2 border-blue-200">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl md:text-2xl font-bold text-blue-700">
              <AlertCircle className="h-6 w-6 text-blue-600" />
              สรุปข้อมูลวันนี้ (สำหรับผู้ใช้ทั่วไป)
            </CardTitle>
            <CardDescription className="text-sm md:text-base">ภาพรวมสถานการณ์น้ำและสภาพอากาศที่สำคัญ</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-2 md:gap-3 text-sm md:text-base">
            ...
            <div className="flex items-center justify-between">
              ...
              <Badge
                className={cn(
                  "text-xs md:text-sm px-3 py-1 font-bold",
        ...
        <Card className="col-span-full bg-white shadow-lg md:col-span-2 lg:col-span-1 xl:col-span-1">
          ...
          <CardContent>
            <p className="text-2xl md:text-3xl font-semibold text-gray-900">{formattedDate}</p>
            <p className="text-xl md:text-2xl text-gray-700 mt-1">{formattedTime}</p>
          </CardContent>
        </Card>
        ...
        <Card className="col-span-full bg-white shadow-lg md:col-span-2 lg:col-span-2 xl:col-span-2">
          <CardHeader>
            ...
            <CardDescription className="text-sm md:text-base">ข้อมูลข้างขึ้นข้างแรมและน้ำเป็นน้ำตาย</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            <div className="flex items-center justify-between">
              <span className="text-base md:text-lg font-medium">ข้างขึ้นข้างแรม:</span>
              <Badge variant="outline" className="bg-blue-100 text-blue-800 text-sm md:text-base">
                {currentTideData.isWaxingMoon ? "ข้างขึ้น" : "ข้างแรม"}
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-base md:text-lg font-medium">น้ำเป็นน้ำตาย:</span>
              <TooltipProvider>
                <Tooltip>
                    ...
                    <Badge
                      ...
                      className={
                        currentTideData.tideStatus === "น้ำเป็น"
                          ? "bg-blue-100 text-blue-800 text-sm md:text-base px-3 py-1"
                          : "bg-purple-100 text-purple-800 text-sm md:text-base px-3 py-1"
                      }
                    >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-base md:text-lg font-medium">น้ำขึ้นสูงสุด:</p>
                <p className="text-2xl md:text-3xl font-bold text-blue-700">{currentTideData.highTideTime}</p>
              </div>
              <div>
                <p className="text-base md:text-lg font-medium">น้ำลงต่ำสุด:</p>
                <p className="text-2xl md:text-3xl font-bold text-red-700">{currentTideData.lowTideTime}</p>
              </div>
            </div>

            {/* Detailed Tide Data - Collapsible */}
            <Collapsible className="mt-4">
              <CollapsibleTrigger className="flex items-center justify-between w-full text-blue-600 hover:text-blue-800 font-medium text-sm md:text-base">
                ข้อมูลน้ำขึ้นน้ำลงเชิงลึก
                <ChevronDown className="h-5 w-5 transition-transform data-[state=open]:rotate-180" />
              </CollapsibleTrigger>
              <CollapsibleContent className="mt-2 text-gray-700 grid gap-2 text-sm md:text-base">
                <p>
        ...
        <Card className="col-span-full bg-white shadow-lg md:col-span-2 lg:col-span-1 xl:col-span-1">
          <CardHeader>
            ...
            <CardDescription className="text-sm md:text-base">ข้อมูลสภาพอากาศและคลื่นลมปัจจุบัน</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            <div className="flex items-center gap-2">
              <Thermometer className="h-6 w-6 text-red-500" />
              <span className="text-base md:text-lg font-medium">อุณหภูมิ:</span>
              <span className="text-base md:text-lg">{currentWeatherData.main.temp}°C</span>
            </div>
            <div className="flex items-center gap-2">
              <Thermometer className="h-6 w-6 text-orange-500" />
              <span className="text-base md:text-lg font-medium">รู้สึกเหมือน:</span>
              <span className="text-base md:text-lg">{currentWeatherData.main.feels_like}°C</span>
            </div>
            <div className="flex items-center gap-2">
              <Cloud className="h-6 w-6 text-gray-500" />
              <span className="text-base md:text-lg font-medium">สภาพอากาศ:</span>
              <span className="text-lg md:text-xl font-semibold">{currentWeatherData.weather[0].description}</span>
            </div>
            <div className="flex items-center gap-2">
              <Waves className="h-6 w-6 text-blue-500" />
              <span className="text-base md:text-lg font-medium">ความเร็วลม:</span>
              <span className="text-lg md:text-xl font-semibold">{currentWeatherData.wind.speed} m/s</span>
            </div>
        ...
        <Card className="col-span-full bg-white shadow-lg md:col-span-1 lg:col-span-1 xl:col-span-1">
          <CardHeader>
            ...
            <CardDescription className="text-sm md:text-base">การคาดการณ์ระดับน้ำทะเลหนุนสำหรับวันนี้</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <span className="text-base md:text-lg font-medium">วันนี้หนุนสูงมั้ย:</span>
              <Badge
                variant="outline"
                className={
                  currentTideData.isSeaLevelHighToday
                    ? "bg-red-500 text-white text-base md:text-lg px-4 py-2 font-bold"
                    : "bg-green-500 text-white text-base md:text-lg px-4 py-2 font-bold"
                }
              >
        ...
        <Card className="col-span-full bg-white shadow-lg md:col-span-1 lg:col-span-1 xl:col-span-1">
          <CardHeader>
            ...
            <CardDescription className="text-sm md:text-base">ระดับน้ำที่คำนวณจากปัจจัยต่างๆ</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <span className="text-base md:text-lg font-medium">ระดับน้ำ:</span>
              <span className="text-3xl md:text-4xl font-bold text-purple-700">
                {currentTideData.currentWaterLevel} ม.
              </span>
            </div>
            <div className="mt-2 flex items-center justify-between">
              <span className="text-base md:text-lg font-medium">สถานะ:</span>
              <Badge
                variant="outline"
                className="bg-purple-500 text-white text-sm md:text-base px-3 py-1 font-semibold"
              >
                {currentTideData.waterLevelStatus}
              </Badge>
            </div>
            <p className="mt-4 text-xs md:text-sm text-gray-600">
              *ระดับน้ำนี้คำนวณจากข้อมูลข้างขึ้นข้างแรมและปัจจัยอื่นๆ เพื่อให้ข้อมูลเบื้องต้น
            </p>
