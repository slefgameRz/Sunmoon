...
import { tileStorage, type TileData } from "./tile-storage";
import {
...
export class SunmoonTideSystem {
  ...
  private systemStatus: SystemStatus = {
    engine: {
      type: "javascript",
      optimized: true,
      description: "Harmonic JS engine (37 constituents)",
    },
  ...
  async initialize(): Promise<SystemStatus> {
    ...
    this.systemStatus.engine = {
      ...
      optimized: true,
      description:
        "Client-side harmonic predictor (37 constituents + nodal corrections)",
    };
    ...
    try {
      if (ENABLE_SW && "serviceWorker" in navigator) {
        ...
        if (registration?.active) {
          ...
          console.info(
            "[System] Service Worker registration skipped by manager",
          );
        }
      } else {
        this.systemStatus.serviceWorker = {
        ...
        if (!ENABLE_SW) {
          console.info(
            "[System] Service Worker disabled via NEXT_PUBLIC_ENABLE_SERVICE_WORKER",
          );
        }
      }
    } catch (error) {
    ...
    try {
      const { usage } = await tileStorage.getStorageEstimate();
      ...
      console.log(
        `โœ… IndexedDB ready (${(size / 1024 / 1024).toFixed(2)} MB)`,
      );
    } catch (error) {
  ...
  async predictTide(
    ...
    const waterLevels = timestamps.map((d) =>
      predictWaterLevel(d, location, constituents),
    );
   ...
   */
  async getTile(lat: number, lon: number): Promise<TileData> {
    ...
    try {
      ...
      const payload = (await response.json()) as TileApiResponse;
